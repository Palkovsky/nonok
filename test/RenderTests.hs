...
renderSingleForLoop = testCase "Single for loop"
   (assertEqualIO  "Should render valid output"
      ...
      (feed defaultRenderState "{{for $i in [1,2,3]}} Numer: {- $i}} {{endfor}}")
   )
...
renderNestedForLoop = testCase "Two nested for loops"
   (assertEqualIO "Should render valid output"
      ...
      (feed defaultRenderState "{{for $i in [1,2,3]}}{{for $j in 'abc'}}({-$i}}, {-$j}}){{endfor}}{{endfor}}")
   )
...
renderForLoopWithReferenceIterator = testCase "For loop with referenced iterator"
  (assertEqualIO "Should render valid output"
     ...
     (feed defaultRenderState "{{for $i in [['abc'],['def'],['ghi']]}}{{for $j in $i}}{-$j}}{{endfor}}{{endfor}}")
  )
...
renderForWithOutsideReferenceDeclaration = testCase "For loop with referenced iterator declared in let"
  (assertEqualIO "Should render valid output"
     (return $ Right "abcdefghi")
     (feed defaultRenderState "{{let $arr=[['abc'],['def'],['ghi']]}}{{for $i in $arr}}{{for $j in $i}}{-$j}}{{endfor}}{{endfor}}")
  )
...
loopThroughFunctionResult = testCase "For loop through function result"
  (assertEqualIO "Should render valid output"
     ...
     (feed defaultRenderState "{{ let $name='dawid' }}{{ for $char in upper($name)}}{-$char}} {{endfor}}")
  )
...
renderAccessToUndefinedVar = testCase "Error out if accessed undefined var"
 (assertEitherIO "Should error out"
    (return left)
    (feed defaultRenderState "{{for $i in $k}} dawid {{endfor}}")
 )
...
renderOutOfScopeAccess = testCase "Error out if accessed out of scope var"
  (assertEitherIO "Should error out"
     (return left)
     (feed defaultRenderState "{{for $i in [['abc'],['def'],['ghi']]}} {{let $k=20}} {{endfor}} {- $k }}")
  )
...
renderIfStatementWithFunctionResult = testCase "If statement with function call as bool"
    (assertEqualIO "Should render valid output"
        ...
        (feed defaultRenderState "{{ let $name='dawid' }}{{ if upper($name) }}not empty{{endif}}")
    )
...
renderIfInForLoop = testCase "If statement inside for loop"
    (assertEqualIO "Should render valid output"
        ...
        (feed defaultRenderState "{{ for $char in 'dawid' }}{{ if equal($char, 'd') }}JESTEM D {{elif equal($char, 'a')}}JESTEM A {{else}}NIEWIEMKIMJESTEM {{endif}}{{endfor}}")
    )
...
renderIncludeFromVar = testCase "Include from var"
 (assertEqualIO "Should render valid output"
    (return $ Right "123")
    (feed defaultRenderState "{{let $i='{{for $i in [1,2,3]}}{-$i}}{{endfor}}'}}{{include $i}}")
 )
...
renderIncludeFromPath = testCase "Include from path"
 (assertEqualIO "Should render valid output"
    (return $ Right "123\n")
    (feed defaultRenderState "{{include 'test/static/include_test_correct.txt'}}")
 )
...
renderIncludeLocalInheritanceError = testCase "Inheriting local variables"
  (assertEitherIO "Should error out"
     (return left)
     (feed defaultRenderState "{{let $j=21, $i='{-$j}}'}}{{include $i}}")
  )
...
renderIncludeFromVarWithError = testCase "Include from var with error"
  (assertEitherIO "Should error out"
     (return left)
     (feed defaultRenderState "{{let $i='{{for $i in [1,2,3]}}{-$i}}'}}{{include $i}}")
  )
...
renderIncludeFromPathWithError = testCase "Include from path with error"
 (assertEitherIO "Should error out"
    (return left)
    (feed defaultRenderState "{{include 'test/static/include_test_error.txt'}}")
 )
...
renderRaw = testCase "Rendering contents of raw tag"
    ...
    (feed defaultRenderState " {{raw }}{{if $x}}xxx{{endif}}{{endraw }} ")
    )
...
renderComment = testCase "Skipping contents of comment tag"
    (assertEqualIO "Should render valid output"
         (return $ Right "  ")
    (feed defaultRenderState " {{ comment }}  {   {i  f $x }}x xx{ { e n dif}}{{ endcomment }} ")
    )
